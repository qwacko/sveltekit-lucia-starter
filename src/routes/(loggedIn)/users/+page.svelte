				...
				<div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-lg hover:shadow-xl transition-all duration-200">
					<div class="flex items-center justify-between mb-4">
						<div class="flex items-center gap-3">
							...
							<div>
								<h3 class="font-semibold text-gray-900 dark:text-gray-100">{currentUser.name || 'User'}</h3>
								{#if currentUser.admin}
									<span class="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 px-2 py-1 rounded-full font-medium">Admin</span>
								{/if}
							</div>
						</div>
					</div>
